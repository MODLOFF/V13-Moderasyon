...
module.exports = class Erkek extends Command {
    ...
    async run(client, interaction) {
        ...
        if(!member.roles.cache.has(roles.erkek)) {
            setTimeout(() => {
                member.roles.add(roles.erkek)          
         }, 1000);
        await member.roles.remove(roles.unregister)
        }
        interaction.reply({content: `${emojis.ok} ${member} adlı kişi başarılı şekilde **Erkek** olarak kayıt edildi.`})

        Data.findOne({user: member.id}, async(err,res) => {
            if(!res) {
              let arr = []
              arr.push({isim: member.displayName, rol: roles.erkek, yetkili: interaction.member.id})
              ...
              res.isimler.push({isim: member.displayName, rol: roles.erkek, yetkili: interaction.member.id})
